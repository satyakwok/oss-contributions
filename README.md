...
### Execution and ABI tooling: alloy / Foundry
...
- [Sentrix Chain](https://github.com/sentrix-labs/sentrix): a Rust EVM Layer-1 with a
  native Rust node, EVM execution via [`revm`](https://github.com/bluealloy/revm), BFT
  consensus, and full RPC / explorer / faucet / validator tooling.
- [reliakit](https://github.com/satyakwok/reliakit): a zero-dependency Rust reliability
  toolkit (`no_std`-friendly, no `unsafe`) covering retry/backoff, timeout, rate
  limiting, circuit breaker, bounded collections, secret redaction, and health checks.
